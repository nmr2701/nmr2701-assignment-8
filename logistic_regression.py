...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    X2 = np.random.multivariate_normal(mean=[1 + distance, 1 + distance], cov=covariance_matrix, size=n_samples)  # Ensure shifting
...
def do_experiments(start, end, step_num):
    ...
    plt.subplot(3, 3, 4)
    slopes = [b1 / b2 if b2 != 0 else 0 for b1, b2 in zip(beta1_list, beta2_list)]  # Avoid division by zero
    ...
    intercept_ratios = [b0 / b2 if b2 != 0 else 0 for b0, b2 in zip(beta0_list, beta2_list)]  # Avoid division by zero
    plt.subplot(3, 3, 5)
    plt.plot(shift_distances, intercept_ratios, marker='o')
